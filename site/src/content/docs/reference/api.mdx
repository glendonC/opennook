...
- **Layout metrics** - `NookLayout`, the public chrome dimension constants
  (expanded width, edge padding, compact-slot size, breadcrumb width). Source:
...
- `NookStyle` - corner radii and the built-in animation curves.
...
[Theming](/guides/theming/#chrome-shape-animation-and-width). The remaining
surface types (`Nook`, `NookState`, `NookBackdrop`, `NookHoverBehavior`,
`NookContentInsets`, `NookFeedback`) live under `Sources/NookSurface/`.
